...
object Usage extends App {
  ...
  for (m ← movies.results) {
    ...
    Await.result(
        tmdbClient.downloadPoster(
            movie, s"${System.getProperty("user.home")}/poster-${m.id}.jpg"),
        timeout)
    ...
    val release = releases.countries
      ...
      .release_date
    tmdbClient.log.info(
        s"OK got a movie ${movie.title}. Director name is ${director.getOrElse(noCrew).name} and US released date is $release")
  }
